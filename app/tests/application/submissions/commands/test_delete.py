...
@pytest.mark.asyncio
async def test_delete_submission_command_success(
    mediator: Mediator,
    ...
    await mediator.handle_command(
        DeleteSubmissionCommand(submission_id=created_submission.oid),
    )
...
@pytest.mark.asyncio
async def test_delete_submission_command_not_found(
    mediator: Mediator,
):
    ...
    with pytest.raises(SubmissionNotFoundException) as exc_info:
        await mediator.handle_command(
            DeleteSubmissionCommand(submission_id=non_existent_id),
        )
